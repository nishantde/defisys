		...
		if (input_password.equals(PASSWORD)) {
			...
			try {
				...
				if(upload.equals(args[0])) {
					...
					ProcessBuilder encrypt = new ProcessBuilder("python", "encrypt.py", args[1], args[2]).inheritIO();
					Process p2 = encrypt.start();
					p2.waitFor();
				...
				if(download.equals(args[0])) {
					...
					ProcessBuilder decrypt = new ProcessBuilder("python", "decrypt.py", args[1], args[2]).inheritIO();
					Process p_download = decrypt.start();
					p_download.waitFor();
